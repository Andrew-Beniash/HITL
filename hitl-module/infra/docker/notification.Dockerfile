...
RUN pnpm --filter @hitl/notification build
RUN pnpm deploy --filter @hitl/notification --prod /deploy/notification

# ── Stage 4: production image ─────────────────────────────────────────────────
FROM node:22-alpine AS prod
RUN addgroup -S hitl && adduser -S hitl -G hitl
WORKDIR /app
COPY --from=build --chown=hitl:hitl /deploy/notification ./
USER hitl
