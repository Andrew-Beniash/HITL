...
RUN pnpm --filter @hitl/collaboration build
RUN pnpm deploy --filter @hitl/collaboration --prod /deploy/collaboration

# ── Stage 4: production image ─────────────────────────────────────────────────
FROM node:22-alpine AS prod
RUN addgroup -S hitl && adduser -S hitl -G hitl
WORKDIR /app
COPY --from=build --chown=hitl:hitl /deploy/collaboration ./
USER hitl
